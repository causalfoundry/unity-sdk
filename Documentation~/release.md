...
`https://github.com/causalfoundry/unity-sdk.git#v1.0.7`; do not add a `?path=` query.
...
2. Create an immutable annotated tag matching the manifest version, for example `v1.0.7`.
3. Push the commit and tag, then create release notes from `CHANGELOG.md`.
4. Verify a blank Unity project can install
   `https://github.com/causalfoundry/unity-sdk.git#v1.0.7` (updated for the release version) and that
